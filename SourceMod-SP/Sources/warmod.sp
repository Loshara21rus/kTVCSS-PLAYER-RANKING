...
ResetMatch(bool:silent)
{
	...
	if (!silent)
	{
		for (new x = 1; x <= 3; x++)
		{
			PrintToChatAll("\x03kTVCSS: \x04%t", "Match Reset");
		}
...
ResetHalf(bool:silent)
{
	...
	if (!silent)
	{
		for (new x = 1; x <= 3; x++)
		{
			PrintToChatAll("\x03kTVCSS: \x04%t", "Half Reset");
		}
		ServerCommand("mp_restartgame 1");
	}
}
...
public Action:ActiveToggle(client, args)
{
	...
	if (GetConVarBool(g_h_active))
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Set Inactive");
		}
		else
		{
	...
	else
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Set Active");
		}
		else
		{
...
public Action:ChangeMinReady(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Set Minready", minready);
		}
		else
		{
	...
	else
	{
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04wm_min_ready = %d", GetConVarInt(g_h_min_ready));
		}
		else
		{
...
public Action:ChangeMaxRounds(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Set Maxrounds", maxrounds);
		}
		else
		{
	...
	else
	{
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04wm_max_rounds = %d", GetConVarInt(g_h_max_rounds));
		}
		else
		{
...
public Action:ChangePassword(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Set Password", new_password);
		}
		else
		{
	...
	else
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04sv_password = '%s'", passwd);
		}
		else
		{
...
ReadyUp(client)
{
	...
	if (g_player_list[client] != PLAYER_READY)
	{
		...
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Not on Team");
		}
	}
	else
	{
		PrintToChat(client, "\x03kTVCSS: \x04%t", "Already Ready");
	}
}
...
ReadyDown(client)
{
	...
	{
		PrintToChat(client, "\x03kTVCSS: \x04%t", "Already Not Ready");
	}
}
...
public Action:ForceAllReady(client, args)
{
	...
	if (g_ready_enabled)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Forced Ready");
		}
		else
		{
	...
	else
	{
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Ready System Disabled2");
		}
		else
		{
...
public Action:ForceAllUnready(client, args)
{
	...
	if (g_ready_enabled)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Forced Not Ready");
		}
		else
		{
...
ShowScore(client)
{
	...
	{
		PrintToChat(client, "\x03kTVCSS: \x04%t", "Match Not In Progress");
	}
...
DisplayScore(client, msgindex, bool:priv)
{
	...
	if (msgindex == 0) // standard play score
	{
		...
		if (priv)
		{
			PrintToChat(client, "\x03kTVCSS: %s", score_msg);
		}
		else
		{
			PrintToChatAll("\x03kTVCSS: %s", score_msg);
		}
	}
	else if (msgindex == 1) // overtime play score
	{
		...
		if (priv)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t%s", "Score Overtime", score_msg);
		}
		else
		{
			PrintToChatAll("\x03kTVCSS: \x04%t%s", "Score Overtime", score_msg);
		}
	}
	else if (msgindex == 2) // overall play score
	{
		...
		if (priv)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t%s", "Score Overall", score_msg);
		}
		else
		{
			PrintToChatAll("\x03kTVCSS: \x04%t%s", "Score Overall", score_msg);
		}
	}
}
...
public Event_Round_Start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_first_half && g_restart)
	{
		ServerCommand("sm_save_reset_score");
		ServerCommand("sm_save_reset_cash");
		g_restart = false;
	}
	else if (!g_first_half && !g_t_knife && g_restart)
	{
		ServerCommand("sm_save_reset_cash");
		g_restart = false;
	}
	else if (g_t_knife && g_restart)
	{
		ServerCommand("sm_save_reset_score");
		ServerCommand("sm_save_reset_cash");
		g_restart = false;
...
public Event_Player_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!g_live && g_match)
	{
		...
		SetDeath(victim, GetDeath(victim) - 1);
	}
	...
	if (GetConVarInt(g_h_score_mode) == 1)
	{
		if (!g_overtime)
		{
			if (GetScore() == GetConVarInt(g_h_max_rounds)) // half time
			{
				...
				{
					PrintToChatAll("\x03kTVCSS: \x04%t", "Half Time");
				}
				else
				{
					PrintToChatAll("\x03kTVCSS: \x04%t", "Half Time Auto Swap");
			...
			else if (GetTScore() == GetConVarInt(g_h_max_rounds) && GetCTScore() == GetConVarInt(g_h_max_rounds)) // complete draw
			{
				...
				{ // max rounds overtime
					...
					DisplayScore(0, 0, false);
					PrintToChatAll("\x03kTVCSS: \x04%t", "Over Time", GetConVarInt(g_h_overtime_mr));
					g_live = false;
					g_t_money = false;
					g_overtime = true;
					g_overtime_mode = 1;
				...
				else if (GetConVarInt(g_h_overtime) == 2) // sudden death overtime
				{
					...
					DisplayScore(0, 0, false);
					PrintToChatAll("\x03kTVCSS: \x04%t", "Over Time Sudden Death");
					g_live = false;
					g_t_money = false;
					g_overtime = true;
					g_overtime_mode = 2;
				...
				else
				{
					...
					PrintToChatAll("\x03kTVCSS: \x04%t", "Full Time");
			...
			else if (!g_playing_out && GetTScore() == GetConVarInt(g_h_max_rounds) + 1 || GetCTScore() == GetConVarInt(g_h_max_rounds) + 1) // full time
			{
				...
				else
				{
					...
					g_playing_out = true;
					PrintToChatAll("\x03kTVCSS: \x04%t", "Playing Out Notice", (GetConVarInt(g_h_max_rounds) * 2));
				}
			}
			else
			{
...
ReadyServ(client, bool:ready, bool:silent, bool:show, bool:priv)
{
	...
	if (ready)
	{
		...
		if (!silent)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Ready");
		}
	}
	else
	{
		...
		if (!silent)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Not Ready");
		}
	}
...
stock LiveOn3Override()
{
	...
	{
		SetConVarBool(g_h_ingame_scores, false);
	...
	if (KvJumpToKey(kv, "live_finished"))
	{
		...
		for (new i = 1; i <= 5; i++)
		{
			...
			if (!StrEqual(text, ""))
			{
				...
				CreateDataTimer(float(lastdelay) + 3.5, PrintToChatDelayed, datapack);
				WritePackString(datapack, text);
				SetConVarBool(g_h_ingame_scores, true);
			}
		}
	}
	CloseHandle(kv);
...
public Action:ChooseTeam(client, args)
{
	...
	{
		PrintToChat(client, "\x03kTVCSS: \x04%t", "Change Teams Midgame");
		return Plugin_Stop;
	}
	...
	{
		PrintToChat(client, "\x03kTVCSS: \x04%t", "Maximum Players");
...
public Action:RestrictBuy(client, args)
{
	...
	{
		PrintToChat(client, "\x03kTVCSS: \x04%t", "Nightvision Blocked");
		return Plugin_Handled;
	}
	...
	if (!g_live && GetConVarBool(g_h_warm_up_grens))
	{
		...
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Grenades Blocked");
			return Plugin_Handled;
		}
	}
...
public Action:CancelKnife(client, args)
{
	...
	if (g_t_knife)
	{
		...
		for (new x = 1; x <= 3; x++)
		{
			PrintToChatAll("\x03kTVCSS: \x04%t", "Knife Round Cancelled");
		}
		if (client == 0)
		{
	...
	else
	{
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Knife Round Inactive");
		}
		else
		{
...
DispInfo(client, String:players_unready[], time)
{
	...
	SetGlobalTransTarget(client);
	g_m_ready_up = CreatePanel();
	Format(Temp, sizeof(Temp), "WarMod - %t.\n%t.", "Ready System", "Advanced Gaming Modifications");
...
IsLive(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			if (client != 0)
			{
				PrintToChat(client, "\x03kTVCSS: \x04%t", "Match Is Live");
			}
			else
			{
				PrintToServer("<WarMod> %T", "Match Is Live", LANG_SERVER);
...
IsActive(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			if (client != 0)
			{
				PrintToChat(client, "\x03kTVCSS: \x04%t", "WarMod Inactive");
			}
			else
			{
...
IsAdminCmd(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "WarMod Rcon Only");
		}
	}
	return false;
}
...
public Action:ChangeT(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Change T Name", name);
		}
		else
		{
	...
	else
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04wm_t = %s", name);
		}
		else
		{
...
public Action:ChangeCT(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04%t", "Change CT Name", name);
		}
		else
		{
	...
	else
	{
		...
		if (client != 0)
		{
			PrintToChat(client, "\x03kTVCSS: \x04wm_ct = %s", name);
		}
		else
		{
...
public Action:SayChat(client, args)
{
	...
	else
	{
		...
		if (text[start_index] == '@' && GetConVarBool(g_h_global_chat))
		{
			...
			{
				PrintToChat(client, "\03kTVCSS: \x04%t", "No Permission");
			}
			return Plugin_Handled;
		}
		...
		else if (StrEqual(command, "info", false) || StrEqual(command, "i", false))
		{
			...
			{
				PrintToChat(client, "\03kTVCSS: \x04%t", "ShowInfo Disabled");
			}
		}
	...
	if ((GetConVarBool(g_h_req_names) && g_ready_enabled && !g_live && (StrEqual(g_t_name, DEFAULT_T_NAME, false) || StrEqual(g_ct_name, DEFAULT_CT_NAME, false))) && (!GetConVarBool(g_h_auto_knife) || g_t_had_knife))
	{
		...
		if (num_ready >= GetConVarInt(g_h_min_ready))
		{
			for (new i = 1; i <= MaxClients; i++)
			{
				if (IsClientInGame(i) && !IsFakeClient(i))
				{
					PrintToChat(i, "\x03kTVCSS: \x04%t", "Names Required");
				}
			}
		}
	}
...
public Action:ShowPluginInfo(Handle:timer, any:client)
{
	if (client != 0 && IsClientConnected(client) && IsClientInGame(client))
	{
		...
		PrintToConsole(client, "");
		PrintToConsole(client, "Created by Twelve-60 of GameTech (www.gametech.com.au) and GoDtm666 SourceTM.com (www.SourceTM.com)");
		PrintToConsole(client, "");
...
public Action:WMVersion(client, args)
{
	if (client == 0)
	{
		PrintToServer("\"wm_version\" = \"%s\"\n - kTVCSS: %s", WM_VERSION, WM_DESCRIPTION);
	}
	else
	{
...
GetDeath(client)
{
	if (g_i_death != -1 || (g_i_death = FindDataMapInfo(client, "m_iDeaths")) != -1)
	{
		return GetEntData(client, g_i_death);
	}
	return false;
}

SetDeath(client, death)
{
	if (g_i_death != -1 || (g_i_death = FindDataMapInfo(client, "m_iDeaths")) != -1)
	{
		SetEntData(client, g_i_death, death);
	}
}
